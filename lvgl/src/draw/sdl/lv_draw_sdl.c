...
void lv_draw_sdl_init_ctx(lv_disp_drv_t * disp_drv, lv_draw_ctx_t * draw_ctx)
{
    ...
    // This is UB. disp_drv->user_data is itself an SDL_Renderer*. There is no reason to dereference
    // this opaque internal SDL type.
    //draw_ctx_sdl->renderer = ((lv_draw_sdl_drv_param_t *) disp_drv->user_data)->renderer;
    draw_ctx_sdl->renderer = (SDL_Renderer*)disp_drv->user_data;
