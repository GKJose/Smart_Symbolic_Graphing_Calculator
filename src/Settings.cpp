...
void createSettingsTab(lv_obj_t* parent){
  ...
  sub_misc_page = lv_menu_page_create(menu,NULL);
  lv_obj_set_style_pad_hor(sub_misc_page,lv_obj_get_style_pad_left(lv_menu_get_main_header(menu),0),0);
  lv_menu_separator_create(sub_misc_page);
  section = lv_menu_section_create(sub_misc_page);
  updateButton = create_button(sub_misc_page,"Update");
  lv_obj_add_event_cb(updateButton,updateButton_cb,LV_EVENT_ALL,NULL);
  ...
  /*create a root page */
	...
	cont = create_text(section,LV_SYMBOL_SETTINGS,"MISC.",LV_MENU_ITEM_BUILDER_VARIANT_1);
	lv_menu_set_load_page_event(menu,cont,sub_misc_page);
