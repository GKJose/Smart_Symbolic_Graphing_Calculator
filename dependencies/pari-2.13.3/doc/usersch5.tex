...
\fun{int}{is_qfb_t}{long t} \kbd{true} iff \kbd{t} is \typ{QFI}
or \typ{QFR}.
...
\fun{GEN}{ZC_u_divexact}{GEN x, ulong y} returns $x/y$ assuming all divisions
are exact.
...
exists and 0 otherwise; if \kbd{flag} is nonzero, return $0$ if $i$ exists
...
\fun{GEN}{gen_factorback}{GEN L, GEN e, void *D, F2 mul, F2 pow} generic form
...
function and the next one is to be accurate even if $s$ has a huge imaginary
